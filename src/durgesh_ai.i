#if SWIGPYTHON
%module(package="durgesh_ai", "threads"=1) durgesh_ai
#else
%module Durgesh_ai
#endif
...
%pragma(java) jniclasscode=%{
    static {
        System.loadLibrary("durgesh_ai_jni");
    }
%}
#endif
...
typedef struct DurgeshAIModel Model;
typedef struct DurgeshAISpkModel SpkModel;
typedef struct DurgeshAIRecognizer KaldiRecognizer;
%}
...
%extend Model {
    Model(const char *model_path)  {
        return durgesh_ai_model_new(model_path);
    }
    ~Model() {
        durgesh_ai_model_free($self);
    }
}

%extend SpkModel {
    SpkModel(const char *model_path)  {
        return durgesh_ai_spk_model_new(model_path);
    }
    ~SpkModel() {
        durgesh_ai_spk_model_free($self);
    }
}

%extend KaldiRecognizer {
    KaldiRecognizer(Model *model, float sample_rate)  {
        return durgesh_ai_recognizer_new(model, sample_rate);
    }
    KaldiRecognizer(Model *model, SpkModel *spk_model, float sample_rate)  {
        return durgesh_ai_recognizer_new_spk(model, spk_model, sample_rate);
    }
    KaldiRecognizer(Model *model, float sample_rate, const char* grammar)  {
        return durgesh_ai_recognizer_new_grm(model, sample_rate, grammar);
    }
    ~KaldiRecognizer() {
        durgesh_ai_recognizer_free($self);
    }

#if SWIGCSHARP
    bool AcceptWaveform(const char *data, int len) {
        return durgesh_ai_recognizer_accept_waveform($self, data, len);
    }
    bool AcceptWaveform(const short *sdata, int len) {
        return durgesh_ai_recognizer_accept_waveform_s($self, sdata, len);
    }
    bool AcceptWaveform(const float *fdata, int len) {
        return durgesh_ai_recognizer_accept_waveform_f($self, fdata, len);
    }
#elif SWIGJAVA
    bool AcceptWaveform(const char *data, int len) {
        return durgesh_ai_recognizer_accept_waveform($self, data, len);
    ...
    bool AcceptWaveform(SWIG_Object ptr) {
        ...
        return durgesh_ai_recognizer_accept_waveform($self, data, length);
    }
#else
    ...
    const char* Result() {
        return durgesh_ai_recognizer_result($self);
    }
    const char* PartialResult() {
        return durgesh_ai_recognizer_partial_result($self);
    }
    const char* FinalResult() {
        return durgesh_ai_recognizer_final_result($self);
    }
}

%rename(SetLogLevel) durgesh_ai_set_log_level;
void durgesh_ai_set_log_level(int level);
